...
test.describe('Basalt migration', () => {
  ...
  test('supports prompt edit, toggle, revert and preview flows', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'Updated workspace prompt' })).toBeVisible()

    const toggle = page.getByRole('switch', { name: 'Toggle Date Context' })
  ...
  test('supports workspace editing, color, directory, open and delete actions', async ({ page }) => {
    ...
    let create = page.getByRole('dialog', { name: 'Create Workspace' })
    await create.getByRole('button', { name: 'Create' }).click()
    await expect(create.getByText('Name is required')).toBeVisible()
    await create.getByRole('button', { name: 'Cancel' }).click()

    await main.getByRole('button', { name: /Add Workspace/ }).click()
    create = page.getByRole('dialog', { name: 'Create Workspace' })
    await expect(create.getByText('Name is required')).not.toBeVisible()
    await create.getByRole('button', { name: 'Browse directory' }).click()
    await expect(create.getByLabel('Path')).toHaveValue('/tmp/browsed-workspace')
    await expect(create.getByLabel('Name')).toHaveValue('browsed-workspace')
